...
All operational tunables live here. Secrets are read from the environment and
are never persisted to disk or exposed to the frontend.
"""
...
def _default_rss_sources() -> List[str]:
    ...
    # Conservative default list of reputable tech-news feeds.
    return [
...
class Settings:
    ...
    api_key: str = os.environ.get("API_KEY", "")  # if empty, auth disabled (dev only)
    ...
    # Rate limit (in-process, single-process only)
...
def reset_settings() -> None:
    """Used by tests to force re-read of environment."""
    global _settings
    _settings = None